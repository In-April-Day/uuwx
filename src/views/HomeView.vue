...
const handleMouseMove = (e: MouseEvent) => {
  ...
  if (glare) {
    const rotateGlareX = ((y - centerY) / centerY) * -20;
    const rotateGlareY = ((x - centerX) / centerX) * 20;
    glare.style.background = `radial-gradient(circle at ${x}px ${y}px, rgba(255,255,255,0.15) 0%, transparent 60%)`;
    glare.style.transform = `perspective(800px) rotateX(${rotateGlareX}deg) rotateY(${rotateGlareY}deg) scale3d(1.02, 1.02, 1.02)`;
  }
};
...
const handleMouseLeave = () => {
  ...
  if (glare) {
    ...
    glare.style.transform =
      "perspective(800px) rotateX(0deg) rotateY(0deg) scale3d(1, 1, 1)";
    glare.style.transition = "all 0.6s cubic-bezier(0.4, 0, 0.2, 1)";
  }
